...
<head>
  <meta charset="UTF-8">
  <title>Storybook v16 - Standup Panel Style</title>
  <style>
    body {
      margin: 0;
      overflow: hidden;
...
<body>
  ...
  <video id="videoPreload2" preload="auto" muted playsinline>
    <source src="BG-Image-2.mp4" type="video/mp4">
  </video>
  ...
  <script>
    ...
    function init() {
      ...
      document.getElementById('skipButton').addEventListener('click', function () {
        ...
        window.location.href = '../index.html?fromStory=true';
      });
    }
    ...
    function fallbackToImage() {
      ...
      loader.load('forest_background.jpg', function (texture) {
    ...
    function preloadPage1Video() {
      ...
      page1VideoElement.addEventListener('error', function (e) {
        ...
        const loader = new THREE.TextureLoader();
        loader.load('page1.jpg', function (texture) {
    ...
    function preloadPage2Video() {
      ...
      page2VideoElement.addEventListener('error', function (e) {
        ...
        // Load static image as fallback
        const loader = new THREE.TextureLoader();
        loader.load('page2.jpg', function (texture) {
    ...
    function preloadPage3Video() {
      ...
      page3VideoElement.addEventListener('error', function (e) {
        ...
        // Load static image as fallback
        const loader = new THREE.TextureLoader();
        loader.load('page3.jpg', function (texture) {
    ...
    function preloadPage4Video() {
      ...
      page4VideoElement.addEventListener('error', function (e) {
        ...
        // Load static image as fallback
        const loader = new THREE.TextureLoader();
        loader.load('page4.jpg', function (texture) {
    ...
    function preloadPage5Video() {
      ...
      page5VideoElement.addEventListener('error', function (e) {
        ...
        // Load static image as fallback
        const loader = new THREE.TextureLoader();
        loader.load('page5.jpg', function (texture) {
    ...
    function preloadPage6Video() {
      ...
      page6VideoElement.addEventListener('error', function (e) {
        ...
        // Load static image as fallback
        const loader = new THREE.TextureLoader();
        loader.load('page6.jpg', function (texture) {
    ...
    function preloadCoverVideo() {
      ...
      coverVideoElement.addEventListener('error', function (e) {
        ...
        // Load static image as fallback
        const loader = new THREE.TextureLoader();
        loader.load('cover.jpg', function (texture) {
    ...
    function turnToPage2Animation() {
      ...
      page2Mesh.rotation.y = 0.2; // 初始轻微旋转
      ...
      const spineMaterial = new THREE.MeshBasicMaterial({ color: 0x555555 });
      const spine = new THREE.Mesh(spineGeometry, spineMaterial);
      spine.position.set(-width / 2 - 0.1, 0, 0); // 固定在左侧边缘
      page2Mesh.add(spine);
      ...
      gsap.to(page2Mesh.rotation, {
        ...
        onComplete: function () {
          ...
          // 动画完成后加入渲染队列最后，确保在最上层
          bookContainer.remove(page2Mesh);
          bookContainer.add(page2Mesh);

          isAnimating = false;
          console.log('切换到page2视频完成，可点击继续到第三页');
        }
      });
    ...
    function turnToPage3Animation() {
      ...
      page3Mesh.rotation.y = 0.2; // 初始轻微旋转
      ...
      gsap.to(page3Mesh.rotation, {
        ...
        onComplete: function () {
          ...
          isAnimating = false;
          console.log('切换到page3视频完成，可点击继续到第四页');
        }
      });
    ...
    function turnToPage4Animation() {
      ...
      page4Mesh.rotation.y = 0.2; // 初始轻微旋转
      ...
      gsap.to(page4Mesh.rotation, {
        ...
        onComplete: function () {
          ...
          isAnimating = false;
          console.log('切换到page4视频完成，可点击继续到第五页');
        }
      });
    ...
    function turnToPage5Animation() {
      ...
      page5Mesh.rotation.y = 0.2; // 初始轻微旋转
      ...
      gsap.to(page5Mesh.rotation, {
        ...
        onComplete: function () {
          ...
          isAnimating = false;
          console.log('切换到page5视频完成，这是最后一页');
        }
      });
    ...
    function turnToPage6Animation() {
      ...
      page6Mesh.rotation.y = 0.2; // 初始轻微旋转
      ...
      gsap.to(page6Mesh.rotation, {
        ...
        onComplete: function () {
          ...
          isAnimating = false;
          console.log('切换到page6视频完成，这是最后一页');
        }
      });
    ...
    // 处理页面焦点 - 页面的放大、缩小以及点击缩小状态页面时跳转到对应页面
    // clickedObject: 被点击的页面对象
    function handlePageFocus(clickedObject) {
      // 如果正在动画中，忽略点击
      if (isAnimating) return;

      console.log("处理页面焦点，点击对象:", clickedObject.userData.type);

      // 如果点击的是当前已聚焦(放大)的对象，执行重置操作，使所有页面恢复正常大小
      ...
      // 这里不应该被执行到，但为了健壮性保留
      console.warn("处理了未识别的对象:", clickedObject);
    }

    // 重置到封面初始状态的函数
    function resetToInitialCoverState() {
      console.log("重置到封面初始状态");
      isAnimating = true;
      ...
      if (coverVideoElement) {
        coverVideoElement.currentTime = 0;
        coverVideoElement.play().catch(e => console.error('无法重新播放封面视频:', e));
      }
    ...
    function resetAllPages() {
      if (!focusedPage) {
        console.log("没有聚焦页面，不需要重置");
        return;
      }

      // 防止动画过程中调用
      if (isAnimating) {
        console.log("动画中，忽略重置请求");
        return;
      }

      console.log("开始重置所有页面");
      isAnimating = true;

      // 保存当前页码
      const savedCurrentPage = currentPage;
      console.log("记录当前页面状态:", savedCurrentPage);
      ...
      // 调用directResetToPage完成重置
      if (targetMesh) {
        directResetToPage(savedCurrentPage, targetMesh);
      } else {
        console.error("无法找到目标页面，重置失败");
        isAnimating = false;
      }
    }
    ...
    // 添加一个简单的调试函数，以便在控制台检查页面位置
    function logPagePositions() {
      console.log('--- 页面位置状态 ---');
      if (coverMesh) console.log('封面:', coverMesh.position, coverMesh.scale);
      if (pageMesh) console.log('第一页:', pageMesh.position, pageMesh.scale);
      if (page2Mesh) console.log('第二页:', page2Mesh.position, page2Mesh.scale);
      if (page3Mesh) console.log('第三页:', page3Mesh.position, page3Mesh.scale);
      if (page4Mesh) console.log('第四页:', page4Mesh.position, page4Mesh.scale);
      if (page5Mesh) console.log('第五页:', page5Mesh.position, page5Mesh.scale);
      if (page6Mesh) console.log('第六页:', page6Mesh.position, page6Mesh.scale);
      console.log('当前聚焦页面:', focusedPage ? focusedPage.userData.type || '未知' : '无');
      console.log('------------------');
    }
    ...
    function focusPage(page) {
      ...
      bookContainer.add(page); // 重新添加会使其在渲染顺序中最后渲染，即最上层
      ...
      // 其他页面向后移动并缩小到0.9倍，确保被选中页面覆盖
      otherPages.forEach((otherPage, index) => {
        // 计算位置，使其均匀分布在主页面后方
        const angle = (index / otherPages.length) * Math.PI * 0.6; // 小于半圆的分布，集中在左侧
        const offsetX = -4 - Math.sin(angle) * 3; // 负值使页面在左侧
        const offsetZ = -3 - Math.cos(angle) * 2; // 所有页面在后方
    ...
    function updatePagesVisibility() {
      console.log("更新页面可见性，当前页面：", currentPage);
      // 根据当前页码设置页面可见性
      if (currentPage === 0) { // 封面
        if (coverMesh) coverMesh.visible = true;
        if (pageMesh) pageMesh.visible = false;
    ...
    // 直接恢复到封面状态而不使用resetToInitialCoverState函数(避免递归)
    function directRestoreCoverState() {
      console.log("直接恢复到封面状态");
      ...
      if (coverVideoElement) {
        coverVideoElement.currentTime = 0;
        coverVideoElement.play().catch(e => console.error('无法播放封面视频:', e));
      }
    ...
    function directResetToPage(pageNum, targetMesh) {
      // 添加完整的调试日志
      console.log("===== 开始重置页面 =====");
      console.log("目标页面:", pageNum, "目标网格:", targetMesh ? targetMesh.userData.type : "无");
      console.log("当前页面:", currentPage, "聚焦页面:", focusedPage ? focusedPage.userData.type : "无");

      // 如果正在动画中，忽略重置请求
      if (isAnimating) {
        console.log("动画中，忽略直接重置请求");
        return;
      }
      ...
      // 确保targetMesh是有效的
      if (!targetMesh) {
        console.error("无效的目标网格对象");
        return;
      }

      isAnimating = true;
      console.log("开始重置到页面:", pageNum, "目标网格类型:", targetMesh.userData.type);

      // 设置当前页面 - 根据targetMesh类型确保pageNum正确匹配
      ...
      // 更新当前页码
      currentPage = pageNum;
      console.log("实际跳转页面:", pageNum);

      // 清除聚焦状态
      if (focusedPage) {
        console.log("清除聚焦状态:", focusedPage.userData.type);
        focusedPage = null;
      }
      ...
      console.log("参考位置:", coverPos, "参考大小:", coverScale);
      ...
      // 非封面页面处理
      console.log("处理非封面页面:", pageNum);

      // 设置页面可见性 - 确保当前页面及之前的页面都可见
      console.log("设置页面可见性:");
      if (coverMesh) {
        coverMesh.visible = true;
        console.log("- 封面页面可见");
      ...
      if (pageMesh) {
        ...
        console.log(`- 第一页${pageNum >= 1 ? '可见' : '不可见'}`);
      }

      if (page2Mesh) {
        page2Mesh.visible = pageNum >= 2;
        console.log(`- 第二页${pageNum >= 2 ? '可见' : '不可见'}`);
      }

      if (page3Mesh) {
        page3Mesh.visible = pageNum >= 3;
        console.log(`- 第三页${pageNum >= 3 ? '可见' : '不可见'}`);
      }

      if (page4Mesh) {
        page4Mesh.visible = pageNum >= 4;
        console.log(`- 第四页${pageNum >= 4 ? '可见' : '不可见'}`);
      }

      if (page5Mesh) {
        page5Mesh.visible = pageNum >= 5;
        console.log(`- 第五页${pageNum >= 5 ? '可见' : '不可见'}`);
      }

      if (page6Mesh) {
        page6Mesh.visible = pageNum >= 6;
        console.log(`- 第六页${pageNum >= 6 ? '可见' : '不可见'}`);
      }
      ...
      if (!currentTargetMesh) {
        console.error("找不到目标页面对象，重置失败:", pageNum);
        isAnimating = false;
        return;
      }
      ...
      // 设置所有非当前页面的位置 - 将它们移到左侧并缩小
      visiblePages.forEach(page => {
        if (page !== currentTargetMesh) {
          console.log("设置非目标页面位置:", page.userData.type);
          gsap.to(page.position, {
            x: -7, // 移到左侧
            y: 0,
            z: -2, // 向后移动
      ...
      // 目标页面居中
      console.log("设置目标页面位置:", currentTargetMesh.userData.type);
      ...
      // 确保目标页面在最上层
      console.log("将目标页面移到最上层:", currentTargetMesh.userData.type);
      bookContainer.remove(currentTargetMesh);
      bookContainer.add(currentTargetMesh);

      // 播放当前页面视频
      console.log("播放当前页面视频:", pageNum);
      switch (pageNum) {
        case 1:
          if (page1VideoElement) {
            page1VideoElement.currentTime = 0;
            page1VideoElement.play().catch(e => console.error('无法播放page1视频:', e));
          }
          break;
        case 2:
          if (page2VideoElement) {
            page2VideoElement.currentTime = 0;
            page2VideoElement.play().catch(e => console.error('无法播放page2视频:', e));
          }
          break;
        case 3:
          if (page3VideoElement) {
            page3VideoElement.currentTime = 0;
            page3VideoElement.play().catch(e => console.error('无法播放page3视频:', e));
          }
          break;
        case 4:
          if (page4VideoElement) {
            page4VideoElement.currentTime = 0;
            page4VideoElement.play().catch(e => console.error('无法播放page4视频:', e));
          }
          break;
        case 5:
          if (page5VideoElement) {
            page5VideoElement.currentTime = 0;
            page5VideoElement.play().catch(e => console.error('无法播放page5视频:', e));
          }
          break;
        case 6:
          if (page6VideoElement) {
            page6VideoElement.currentTime = 0;
            page6VideoElement.play().catch(e => console.error('无法播放page6视频:', e));
          }
          break;
      }
      ...
      gsap.to(camera.position, {
        ...
        onComplete: function () {
          isAnimating = false;
          console.log("页面重置完成，当前页面:", pageNum);
          logPagePositions();
          console.log("===== 结束重置页面 =====");
        }
      });
